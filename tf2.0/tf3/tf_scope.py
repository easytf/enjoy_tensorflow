...
with tf.name_scope("my_name_scope"):
    v1 = tf.Variable(name = "v1", initial_value=[1.0], dtype=tf.float32)
...
print(a)   #add操作张量名 my_name_scope/Add:0,my_name_scope是空间名
   
v3 = tf.Variable(name = "v3", initial_value=[1.0], dtype=tf.float32)
v4 = tf.Variable(2.0, name="v4", dtype=tf.float32)
b = tf.add(v1, v2)
                
print(v3.name)  # my_variable_scope/v3:0
print(v4.name)  # my_variable_scope/v4:0
print(b)   # my_variable_scope/Add:0
